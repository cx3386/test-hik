...
void CImageProcess::run()
{
	...
	while (!quit) {
		...
		//********************霍夫圆检测并分割圆环**************************//
		cv::GaussianBlur(srcImg, srcImg, Size(gs1, gs1), 2, 2);//修改内核大小可更改识别圆的难易，越小越容易
		vector<Vec3f> circles;
		//霍夫圆
		HoughCircles(srcImg, circles, CV_HOUGH_GRADIENT, dp, minDist, param1, param2, 0, 0); //可修改参数调节捕捉圆的准确度
		...
		//将分割后图像大小调整为圆环大小
		Mat imagetemp3(2 * radius0, 2 * radius0, imagetemp0.type()); //定义调整后图像
		...
		// imshow("第一幅图切割效果图", dstImg[0]);
		// imshow("第二幅图切割效果图", dstImg[1]);
		...
		//计算旋转角度
		double data = M.at<double>(0, 1); //输出第1行第2个数
